...
const spec = {
  ...
  paths: {
    ...
    '/v1/aadhaar/kyc': {
      get: {
        summary: 'Mock e-KYC fetch (returns name, DOB, gender, address, mobile from Aadhaar)',
        parameters: [
          {
            name: 'aadhaarNumber',
            in: 'query',
            required: true,
            schema: { type: 'string', minLength: 12, maxLength: 12 },
          },
        ],
        responses: {
          '200': { description: 'KYC record' },
          '400': { description: 'Invalid Aadhaar number' },
        },
      },
    },
    '/v1/beneficiary': {
    ...
    '/v1/intake': {
      post: {
        summary: 'Rich opaque intake (admin-web path)',
        description:
          'Public, rate-limited endpoint for the admin-web PWA. Accepts an arbitrary profile JSON blob, a set of pre-computed matches, and an optional field-worker access code. Persists the profile into beneficiaries.profile_json plus a best-effort structured row in beneficiary_profiles so dashboard aggregates still work.',
        requestBody: {
          required: true,
          content: {
            'application/json': {
                  ...
                  matches: {
                    ...
                    items: {
                      ...
                      properties: {
                        ...
                        eligibilityStatus: {
                          type: 'string',
                          enum: ['eligible', 'partial', 'not_eligible', 'ineligible'],
                        },
                        annualValueInr: { type: 'number', nullable: true },
                        explanationHi: { type: 'string' },
                        explanationEn: { type: 'string' },
                      },
                      required: ['schemeId', 'eligibilityStatus'],
                    },
                  },
                  fieldWorkerCode: { type: 'string' },
                },
                required: ['profile'],
              },
            },
        ...
        responses: {
          '200': { description: 'Existing beneficiary updated' },
          '201': { description: 'Beneficiary created' },
        },
      },
    },
    ...
    '/v1/ingestion/discover': {
      post: { summary: 'Run URL discovery from seed sources + sitemaps', responses: { '200': { description: 'Discovery results' } } },
    },
    '/v1/ingestion/fetch': {
      post: { summary: 'Fetch up to 20 URLs and store source documents', responses: { '200': { description: 'Fetch results' } } },
    },
    '/v1/ingestion/extract': {
      post: { summary: 'Extract scheme candidates from source documents', responses: { '200': { description: 'Extraction results' } } },
    },
    '/v1/ingestion/normalize': {
      post: { summary: 'Normalize candidates into registry-shaped schemes', responses: { '200': { description: 'Normalized results' } } },
    },
    '/v1/ingestion/publish': {
      post: { summary: 'Publish normalized schemes to canonical store', responses: { '200': { description: 'Published results' } } },
    },
    '/v1/ingestion/run-full': {
      post: { summary: 'End-to-end pipeline: discover → fetch → extract → normalize → publish', responses: { '200': { description: 'Pipeline results' } } },
    },
    '/v1/ingestion/stats': {
      get: { summary: 'Ingestion pipeline statistics (source docs, candidates, canonicals)', responses: { '200': { description: 'Stats' } } },
    },
  },
};
